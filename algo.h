...
int dynamic_workload_tail=0;

//----------------------------PFS Variable---------------------------
struct UpdateManager_Worker_{
    int inacc_idx_map_flag;
    int Available_cores;
    int Total_cores;
    bool Worker_needs_workload;
    bool Update_Manager_is_awake;
    bool TaskFinish;
    std::mutex lck;
} parallel_UM_W_status;
...
struct DY_worktask{
    ...
    double error_after;
    double error_temp;
    int move_BehindOrAfter; //1 represents move behind, 0 represents can't simplely move behind
    int idx_number;
    int query_number;
    int update_start;
    int update_end;
    int update_number;
    int response_time_idx;
};

vector<vector<double>> inacc_idx_all;
vector<vector<double>> inacc_idx_SC;
vector<vector<int>> rw_idx_all;

const int queueLength=2000000;
//MPMCQueue<DY_worktask> main_queue_PFS(queueLength);
rigtorp::MPMCQueue<DY_worktask> main_queue_PFS(queueLength);
//Variables for shuffling controller(Update Manager)
struct Batch_workload_record{
    vector<int> node_type; //record whether a node relates a update or not
    //vector<int> source;
    vector<int> update_start;
    vector<int> update_end;
    int head;
    int tail;
    std::mutex lck;
} UpdateManager_batch_recorder, Workers_batch_recorder;

map<int, vector<double>> inacc_idx_map_UM;
struct inacc_idx_forWorkers{
    map<int, vector<double>> inacc;
    int tail;
} inacc_idx_map_worker;
...
inline void fora_setting(int n, long long m){
    ...
    config.rmax = 1.0/((2+2*config.epsilon)*log(2/config.pfail)/config.delta/config.epsilon/config.epsilon/config.theta/config.theta);
    config.rmax *= config.rmax_scale;
    INFO(config.epsilon*sqrt(config.delta/3/m/log(2/config.pfail)));
    INFO(1.0/((2+2*config.epsilon)*log(2/config.pfail)/config.delta/config.epsilon/config.epsilon));
    INFO(config.theta);
