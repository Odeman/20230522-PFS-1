...
//---------------for shuffling controller-------------------------
    std::vector<VertexIdType> out_degrees_for_shuffling_controller;
public:
...
//----------for Shuffling Controller and Workers----------------------------
    inline void update_in_neighbor(VertexIdType u, VertexIdType v) {
        VertexIdType index;
        ...
        out_degrees_for_shuffling_controller[u]++;
    }
    inline void update_out_neighbor(VertexIdType u, VertexIdType v) {
        VertexIdType index = start_pos_in_out_neighbor_lists[u];
        out_neighbors_lists.emplace(out_neighbors_lists.begin() + index, v);
        for (VertexIdType node = u + 1; node < numOfVertices + 2; node ++){
            start_pos_in_out_neighbor_lists[node] = start_pos_in_out_neighbor_lists[node] + 1;
    ...
    inline void generate_out_degree(){
        out_degrees_for_shuffling_controller.resize(numOfVertices+10, 0);
        int graph_n=numOfVertices;
        for(int i=0; i<graph_n; i++){
            VertexIdType idx_start_next = start_pos_in_out_neighbor_lists[i];
            VertexIdType idx_end_next = start_pos_in_out_neighbor_lists[i+1];
            VertexIdType degree_next = idx_end_next - idx_start_next;
            out_degrees_for_shuffling_controller[i]=degree_next;
        }
    }
    inline int get_out_degree_shuffling_controller(int v){
        return out_degrees_for_shuffling_controller[v];
    }
//----------for Shuffling Controller and Workers----------------------------
...
//        }
    ...
    inline VertexIdType getInNeighbor_PFS(int index){
        return appearance_pos_lists[index];
    }
