...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < argc; i++) {
        ...
        else if (arg == "--query_arrRate"){
            config.query_arrRate = atof(argv[i+1]);
        }
        else if (arg == "--update_arrRate"){
            config.update_arrRate = atof(argv[i+1]);
        }
        else if (arg == "--queue_time"){
            config.queue_time = atof(argv[i+1]);
        }
        else if (arg == "--total_worker_num"){
            config.total_worker_num = atoi(argv[i+1]);
        }
        else if (arg == "--total_shuffling_controller_num"){
            config.total_shuffling_controller_num = atoi(argv[i+1]);
        }
        else if (arg == "--degree_shuffling_PFS"){
            config.degree_shuffling_PFS = atoi(argv[i+1]);
        }
    ...
    else if(config.action == DYNAMIC_SS_QUEUE_WORKLOAD){
        if(config.algo == LAZYUP){
            config.graph_location = config.get_graph_folder();
            NewGraph graph;
            graph.read_file(config.graph_location+"/attribute.txt", config.graph_location+"/graph.txt");
            ...
            generate_queue_workload();

            dynamic_ssquery(graph);
		    cout << "Memory usage (MB):" << get_proc_memory()/1000.0 << endl << endl; 
        }
    }
    else if(config.action == DYNAMIC_SS_TEST_VIC_NUM){
        if(config.algo == LAZYUP){
            config.graph_location = config.get_graph_folder();
            NewGraph graph;
            graph.read_file(config.graph_location+"/attribute.txt", config.graph_location+"/graph.txt");
            ...
            dynamic_test_victim_num(graph);
		    cout << "Memory usage (MB):" << get_proc_memory()/1000.0 << endl << endl; 
        }
    }
    else if(config.action == DYNAMIC_SS_QUEUE_WORKLOAD_PARALLEL){
        if(config.algo == LAZYUP){
            config.graph_location = config.get_graph_folder();
            NewGraph graph;
            graph.read_file(config.graph_location+"/attribute.txt", config.graph_location+"/graph.txt");
            graph.generate_out_degree();
            INFO("load graph finish");
            config.delta = 1.0 / graph.getNumOfVertices();
            config.pfail = 1.0 / graph.getNumOfVertices(); 
            config.dbar = double(graph.getNumOfEdges()) / double(graph.getNumOfVertices());
            ...
            if(config.with_rw_idx&&!config.exact)
                deserialize_idx_workers(config.total_worker_num);
            cout<<"deserialize completed"<<endl;

            generate_queue_workload();

            dynamic_ssquery_parallel(graph, config.total_worker_num);
		    cout << "Memory usage (MB):" << get_proc_memory()/1000.0 << endl << endl; 
        }
    }
    else if(config.action == DYNAMIC_SS_QUEUE_WORKLOAD_PARALLEL_SHUFFLING){
        if(config.algo == LAZYUP){
            config.graph_location = config.get_graph_folder();
            NewGraph graph;
            graph.read_file(config.graph_location+"/attribute.txt", config.graph_location+"/graph.txt");
            ...
            dynamic_ssquery_parallel_shuffling(graph, config.total_worker_num, config.total_shuffling_controller_num);
		    cout << "Memory usage (MB):" << get_proc_memory()/1000.0 << endl << endl; 
        }
    }
