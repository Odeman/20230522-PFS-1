...
inline void deserialize_idx(){
    ...
    if(config.alter_idx == 0)
        ...
        ia >> rw_idx_alter;
    rw_idx_all.resize(1);
    for (auto &row : rw_idx_all) { row.reserve(rw_idx.size()); row.assign(rw_idx.size(), 1); }
    //rw_idx_sharing_withLCK.rw_idx_sharing.reserve(rw_idx.size());
    //rw_idx_sharing_withLCK.rw_idx_sharing.assign(rw_idx.size(), 1);
    for(int i=0; i<1; i++){
...
//------------------------------------------------------------
inline void deserialize_idx_workers(int total_worker_num){
    string file_name = get_idx_file_name();
    assert_file_exist("index file", file_name);
    std::ifstream ifs(file_name);
    boost::archive::binary_iarchive ia(ifs);
